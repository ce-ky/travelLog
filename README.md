...
- **地图** — 记录以**水滴形标记**展示（白色=无图，绿色=有图），同一旅途的记录按时间
  连线；缩小时折叠成「旅途」聚合；放大到街道级后点空白处即可就地新增记录。鼠标移入
  任意气泡会有「变大又弹回」的灵动动效；宽屏下点旅途气泡，右侧面板列出该旅途的全部记录。
- **旅途** — 卡片列表，可重命名 / 编辑（同行人和时间）/ 删除。宽屏为左右双栏：点左侧旅途，
  右侧原位展示该旅途的全部记录；窄屏（手机）仍点开整屏详情。
- **浏览** — 按类型筛选，或按同行人浏览（看和每个人一起去过的地方，可增/改名/删同行人）。
- **记录格式** — 默认【图+文】：最多 4 张图 + 正文（≤5000 字），标题可选（填了才展示）。
- **新增记录** — 类型、照片/画上传（可多选，≤4 张）、随笔、时间、位置；50 公里内自动归到最近的旅途。
- **照片自动定位** — 上传带地理标记的照片（如 iPhone 拍摄）时，自动读取其 EXIF 中的
  GPS 位置与拍摄时间并填入，记录随即落到地图上对应位置（未手动设置时才自动填，可随时改）。
...
> **升级已有项目**：多图功能需要执行 `0003_entry_image_paths.sql`（新增 `image_paths` 数组列
> 并从旧的 `image_path` 回填）。不跑这条迁移，多图上传不会真正生效。
